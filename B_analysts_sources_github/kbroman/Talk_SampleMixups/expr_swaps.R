...
maincolor <- broman::brocolors("crayons")["Cornflower"]
bgcolor <- broman::brocolors("bg")
gray <- bgcolor
darkgray <- "gray30"
CCcolor <- c(rgb(240,240,  0,maxColorValue=255),
...
hetcolor <- rgb(0, 200, 0, maxColorValue=255)
f2color <- c(CCcolor[1], hetcolor, CCcolor[5])
sexcolor <- c(rgb(255,80,80, maxColorValue=255),
              CCcolor[5])
distcolor <- c(darkgray, hetcolor, sexcolor[1])
black <- darkgray


attach("cache/tissue_text.RData")
...
pdf("../Figs/expr_swaps.pdf", height=7.5, width=10, pointsize=18)
par(mfcol=c(3,2), mar=c(1.1, 1.1, 3.1, 1.1))
par(bg=bgcolor, fg="white", col.axis="white", col="white")
for(i in 1:6) {
