...
setValidity("report_mig_char",function(object)
	{
	  ...
	  if (!rep4) retValue=gettext("This report should be for just one taxa")
...
setMethod("choice_c",signature=signature("report_mig_char"),definition=function(object,
	  ...
	  r_mig_char@stage<-charge_with_filter(object=r_mig_char@stage,r_mig_char@dc@dc_selectionne,r_mig_char@taxa@data$tax_code)	
	  ...
	  r_mig_char@parquan<-charge_with_filter(object=r_mig_char@parquan,dc_selectionne=r_mig_char@dc@dc_selectionne,
		  taxa_selectionne=r_mig_char@taxa@data$tax_code,
		  stage_selectionne=r_mig_char@stage@data$std_code)	
	  if (!is.null(parquan))
	  ...
	  r_mig_char@parqual<-charge_with_filter(object=r_mig_char@parqual,r_mig_char@dc@dc_selectionne,r_mig_char@taxa@data$tax_code,r_mig_char@stage@data$std_code)	
	  if (!is.null(parqual)){
	  ...
	  r_mig_char@horodatedebut<-choice_c(object=r_mig_char@horodatedebut,
		  nomassign="bmC_date_debut",
		  funoutlabel=gettext("Beginning date has been chosen\n",domain="R-stacomiR"),
		  horodate=horodatedebut, 
		  silent=silent)
	  r_mig_char@horodatefin<-choice_c(r_mig_char@horodatefin,
		  nomassign="bmC_date_fin",
		  funoutlabel=gettext("Ending date has been chosen\n",domain="R-stacomiR"),
		  horodate=horodatefin,
		  silent=silent)
	  r_mig_char@echantillon<-charge(r_mig_char@echantillon,vecteur=c(gettext("with",domain="R-stacomiR"),gettext("without",domain="R-stacomiR")),
		  label="essai",
...
#' @keywords internal
setMethod("charge",signature=signature("report_mig_char"),definition=function(object,silent=FALSE){ 
	  r_mig_char<-object 
	  ...
	  if (length(parquan)==0 & length(parqual)==0) {
		...
		if (length(parqual)!=0) {
		  ...
		  req@sql=paste("SELECT ",
			  ...
			  " WHERE ope_dic_identifiant in ",vector_to_listsql(r_mig_char@dc@dc_selectionne),
			  echantillons,
			  ...
			  ,sep="")
		  r_mig_char@data[["parqual"]]<-connect(req)@query
		}# end if (parqual)
		if (length(parquan)!=0) {
		  ...
		  r_mig_char@data[["parquan"]]<-connect(req)@query				
		}# end if (parquan)
	  }# end else		
	  return(r_mig_char)
	})
...
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
#' @export
setMethod("setasqualitative",signature=signature("report_mig_char"),definition=function(object,par,silent=FALSE,...) {
	  r_mig_char<-object
	  # par <-'A124'
	  #========= initial checks ================
...
setMethod("calcule",signature=signature("report_mig_char"),definition=function(object,silent=FALSE){ 
	  r_mig_char<-object
...
#' @export
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
setMethod("plot",signature=signature(x="report_mig_char",y="missing"),definition=function(x,color_parm=NULL,plot.type="qual",silent=FALSE,...){ 
	  r_mig_char<-x
	  if (nrow(r_mig_char@calcdata)==0) stop("no data in calcdata, have you forgotten to run calculations ?")
...
#' @export
setMethod("summary",signature=signature(object="report_mig_char"),definition=function(object,silent=FALSE,...){
	  r_mig_char<-object
