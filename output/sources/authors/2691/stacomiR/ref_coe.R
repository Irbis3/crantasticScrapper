...
#' @keywords classes
setClass(Class="ref_coe",representation=
...
setMethod("supprime",signature=signature("ref_coe"),
	definition=function(object,tax,std,silent=FALSE)
	{ 
	  #object<-r_gew@coe;tax=2038;std="CIV"
	  # getting the data to import
	  
	  # here I assume that dc_selectionne will be unique (no report with several dc)
	  #
	  requete=new("RequeteODBCwheredate")
	  requete@baseODBC<-get("baseODBC",envir=envir_stacomi)
	  requete@datedebut<-object@datedebut
