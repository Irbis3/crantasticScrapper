...
setMethod("connect",signature=signature("report_ope"),definition=function(object,silent=FALSE) {
	  # object<-report_ope
...
#' @author cedric.briand
#' @keywords internal
setMethod("charge",signature=signature("report_ope"),definition=function(object,silent=FALSE) {
	  # object<-report_ope
	  ...
	  if (exists("report_ope_date_debut",envir=envir_stacomi)) {
		object@horodatedebut@horodate<-get("report_ope_date_debut",envir=envir_stacomi)
	  } else {
		funout(gettext("You need to choose the starting date\n",domain="R-stacomiR"),arret=TRUE)
	  }
	  
	  if (exists("report_ope_date_fin",envir=envir_stacomi)) {
		object@horodatefin@horodate<-get("report_ope_date_fin",envir=envir_stacomi)
	  } else {
