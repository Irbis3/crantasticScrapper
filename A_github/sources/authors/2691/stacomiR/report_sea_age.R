...
setValidity("report_sea_age",function(object)
	{
	  ...
	  label1<-'report_sea_age should only be for salmon (tax_code=2220)'
	  rep2=all(object@stage@data$std_code%in%c('5','11','BEC','BER','IND'))
	  label2<-'Only stages 5,11,BEC,BER,IND should be used in report_sea_age'
...
#' @keywords internal
setMethod("charge",signature=signature("report_sea_age"),definition=function(object,h) {
	  ...
	  if (exists("r_seaa_date_debut",envir_stacomi)) {
		object@horodatedebut@horodate<-get("r_seaa_date_debut",envir_stacomi)
	  } else {
...
#' @export
setMethod("choice_c",signature=signature("report_sea_age"),definition=function(object,
		dc,
		taxa=2220,
	  ...
	  r_seaa@taxa<-charge_with_filter(object=r_seaa@taxa,r_seaa@dc@dc_selectionne)			
	  r_seaa@taxa<-choice_c(r_seaa@taxa,taxa)
	  r_seaa@stage<-charge_with_filter(object=r_seaa@stage,r_seaa@dc@dc_selectionne,r_seaa@taxa@data$tax_code)	
	  r_seaa@stage<-choice_c(r_seaa@stage,stage,silent=silent)
	  r_seaa@par<-charge_with_filter(object=r_seaa@par,r_seaa@dc@dc_selectionne,r_seaa@taxa@data$tax_code,r_seaa@stage@data$std_code)	
	  r_seaa@par<-choice_c(r_seaa@par,par,silent=silent)
	  r_seaa@horodatedebut<-choice_c(object=r_seaa@horodatedebut,
		  nomassign="r_seaa_date_debut",
		  funoutlabel=gettext("Beginning date has been chosen\n",domain="R-stacomiR"),
		  horodate=horodatedebut, 
		  silent=silent)
	  r_seaa@horodatefin<-choice_c(r_seaa@horodatefin,
		  nomassign="r_seaa_date_fin",
		  funoutlabel=gettext("Ending date has been chosen\n",domain="R-stacomiR"),
		  horodate=horodatefin,
		  silent=silent)
	  r_seaa@limit1hm<-choice_c(r_seaa@limit1hm,as.character(limit1hm),"limit1hm")
	  r_seaa@limit2hm<-choice_c(r_seaa@limit2hm,as.character(limit2hm),"limit2hm")
	  validObject(r_seaa)
	  return(r_seaa)
	})
...
#' @export
setMethod("calcule",signature=signature("report_sea_age"),definition=function(object,silent) {
	  ...
	  if (is.na(as.numeric(r_seaa@limit1hm@label))) stop("internal error")
	  # if no value, a dummy value of 2m
	  if (is.na(as.numeric(r_seaa@limit2hm@label))) r_seaa@limit2hm@label<-2000
	  lescoupes<-c(0,as.numeric(r_seaa@limit1hm@label),as.numeric(r_seaa@limit2hm@label),2001)
...
#' @export
setMethod("plot", signature(x = "report_sea_age", y = "missing"), definition=function(x, plot.type="1", silent=FALSE){ 
...
#' @export
setMethod("summary",signature=signature(object="report_sea_age"),definition=function(object,silent=FALSE,...){
	  r_seaa<-object
	  dat<-r_seaa@calcdata[["data"]]	
	  if (nrow(dat)==0) {   
		if (!silent) funout(gettext("You need to launch computation first, clic on calc\n",domain="R-stacomiR"),arret=TRUE)
...
setMethod("write_database",signature=signature("report_sea_age"),definition=function(object,silent=TRUE){
	  ...
	  if (class(r_seaa)!="report_sea_age") stop("the r_seaa should be of class report_sea_age")
	  ...
	  comment=gettextf("Age calculated from the size of fish compared to reference value %s for the limit between 1 sea winter and 2 sea winter fish, and %s for the limit between 2 sea winter fish and 3 sea winter fish",r_seaa@limit1hm@label,r_seaa@limit2hm@label)
	  bam=data.frame(
...
#' @export
setMethod("print",signature=signature("report_sea_age"),definition=function(x,...){ 
	  sortie1<-"r_seaa=new('report_sea_age')"
	  sortie2<-stringr::str_c("r_seaa=choice_c(r_seaa,",
...
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
#' @keywords internal
funplotreport_sea_age = function(h,...) {
  r_seaa<-get(x="r_seaa",envir=envir_stacomi)
  r_seaa<-charge(r_seaa)
  r_seaa<-connect(r_seaa)
  r_seaa<-calcule(r_seaa)
  #plot.type is determined by button in h$action
  the_plot<-plot(r_seaa,plot.type=h$action)
  print(the_plot)
}
...
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
#' @keywords internal
funtablereport_sea_age = function(h,...) {
  r_seaa=charge(r_seaa)
  r_seaa<-connect(r_seaa)
  r_seaa<-calcule(r_seaa)
  r_seaa<-print(r_seaa)
...
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
#' @export
setMethod("supprime",signature=signature("report_sea_age"),
