...
print.ocProb <- function(x, ...) {print(tail(x$out[[length(x$out)]]))}

plot.ocProb <- function(x, ...) { 
  pr <- x$out[[length(x$out)]]
  yvar <- toupper(as.character(formula(x$w))[2])
  plot(pr[, 2] ~ pr[, 1], type="l", lty=1, # first line
    ylim= range(pr[, -1]), 
    xlab= toupper(x$nam.c),
    ylab= paste("Probability (", yvar, " = J)", sep="")
  ) 
  for (i in 3:ncol(pr)) { # add more lines 
    lines(pr[, i] ~ pr[, 1], type="l", lty=i-1)  
  }  
  abline(v=x$mean.x[x$nam.c], lty=4, col='red'); grid() # mean line  
  legend(x = "topright", legend = x$lev, lty = 1:(ncol(pr) - 1), 
    bg = "white")
}