#' @export
#' 
#' @title MLE Fitting of log-normal Bulk and GPD Tail Extreme Value Mixture Model with Single Continuity Constraint
#'
...
#' 
#' @details The extreme value mixture model with log-normal bulk and GPD tail with continuity at threshold is 
...
#' help for \code{\link[evmix:lognormgpdcon]{dlognormgpdcon}} for details, type \code{help lognormgpdcon}.
...
#' (\code{lnmean}, \code{lnsd}, \code{u}, \code{xi}) if threshold is also estimated and
#' (\code{lnmean}, \code{lnsd}, \code{xi}) for profile likelihood or fixed threshold approach.
#' 
#' Non-positive data are ignored.
#' 
#' @return Log-likelihood is given by \code{\link[evmix:flognormgpdcon]{llognormgpdcon}} and it's
#'   wrappers for negative log-likelihood from \code{\link[evmix:flognormgpdcon]{nllognormgpdcon}}
#'   and \code{\link[evmix:flognormgpdcon]{nlulognormgpdcon}}. Profile likelihood for single
#'   threshold given by \code{\link[evmix:flognormgpdcon]{proflulognormgpdcon}}. Fitting function
#'   \code{\link[evmix:flognormgpdcon]{flognormgpdcon}} returns a simple list with the
#'   following elements
#'
#' \tabular{ll}{
...
#'  \code{lnsd}:      \tab MLE of log-normal standard deviation\cr
...
#'  
#' @aliases flognormgpdcon llognormgpdcon nllognormgpdcon proflulognormgpdcon nlulognormgpdcon
...
# maximum likelihood fitting for log-normal bulk with GPD for upper tail with continuity at threshold
flognormgpdcon <- function(x, phiu = TRUE, useq = NULL, fixedu = FALSE, pvector = NULL,
  ...
  if (is.null(useq)) { # not profile or fixed
    ...
    if (length(useq) != 1) {
      ...
      nllhu = sapply(useq, proflulognormgpdcon, pvector = pvector, x = x, phiu = phiu,
  ...
  if (fixedu) { # fixed threshold (separable) likelihood
    nllh = nlulognormgpdcon(pvector, u, x, phiu)
    if (is.infinite(nllh)) {
      pvector[3] = 0.1
      nllh = nlulognormgpdcon(pvector, u, x, phiu)    
    }
    ...
    fit = optim(par = as.vector(pvector), fn = nlulognormgpdcon, u = u, x = x, phiu = phiu,
    ...
    nllh = nllognormgpdcon(pvector, x, phiu)
    if (is.infinite(nllh)) {
      pvector[4] = 0.1
      nllh = nllognormgpdcon(pvector, x, phiu)    
    }
    ...
    fit = optim(par = as.vector(pvector), fn = nllognormgpdcon, x = x, phiu = phiu,
      finitelik = finitelik, method = method, control = control, hessian = TRUE, ...)    
...
#' @export
#' @aliases flognormgpdcon llognormgpdcon nllognormgpdcon proflulognormgpdcon nlulognormgpdcon
#' @rdname  flognormgpdcon

# log-likelihood function for log-normal bulk with GPD for upper tail with continuity at threshold
llognormgpdcon <- function(x, lnmean = 0, lnsd = 1, u = qlnorm(0.9, lnmean, lnsd),
  ...
  check.inputn(c(length(lnmean), length(lnsd), length(u), length(xi), length(phiu)), allowscalar = TRUE)
...
# negative log-likelihood function for log-normal bulk with GPD for upper tail with continuity at threshold
